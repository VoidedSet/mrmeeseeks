...
class MemoryAgent:
    ...
    async def update_memory(self, key: str, data) -> dict:
        ...
        try:
            ...
            log.warning(f"Failed to propagate memory '{key}' to Chroma DB: {e}")
    ...
    async def fetch_memory(self, keys: list[str]) -> dict:
        ...
        for requested_key in keys:
            ...
            try:
                def query_store():
                    return self.store.search_memories(query=requested_key, container_tag="chat_memory", limit=3)
                
                sm_results = await asyncio.to_thread(query_store)
                facts = [m.get("content") for m in sm_results if m.get("content")]
                if facts:
                    # Merge results with local KV results
                ...
                log.warning(f"Chroma search failed for '{requested_key}': {e}")
