...
def _synthesis_worker():
    ...
    def _init_engine_lazy():
        ...
        try:
            ...
            session = InferenceSession(model_path, providers=[("CUDAExecutionProvider", gpu_opts)])
            _kokoro_engine = Kokoro.from_session(session, voices_path)
            log.info("Kokoro TTS engine initialized lazy ✓")
            return _kokoro_engine
        except Exception as e:
            log.error(f"Lazy Kokoro engine init error: {e}")
            return None
    ...
    while True:
        try:
            ...
            engine = _kokoro_engine or _init_engine_lazy()
            if engine is None:
                log.error("Kokoro engine unavailable for synthesis.")
...
def _play_worker():
    ...
    while True:
        try:
            ...
            if _active_stream is None and stream is not None:
                try:
                    stream.stop()
                    stream.close()
                except Exception:
            ...
            # If main thread cleared _active_stream while we were writing
            if _active_stream is None and stream is not None:
                try:
                    stream.stop()
                    stream.close()
...
async def handle_stop_speak(args: dict) -> dict:
    ...
    # 1. Abort stream if active (let playback thread close it safely)
