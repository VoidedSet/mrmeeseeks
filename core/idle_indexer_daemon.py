...
class IdleIndexerDaemon:
    ...
    def __init__(
        ...
        self.db_path = db_path or os.path.expanduser("~/.meeseeks/indexer_state.db")
    ...
    def _remove_from_index(self, filepath: str):
        """Removes from state DB and attempts to delete from Chroma."""
        abs_path = os.path.abspath(filepath)
        sm_doc_id = self._get_sm_doc_id(abs_path)
        if sm_doc_id:
            self.store.delete_document(sm_doc_id)
        conn = sqlite3.connect(self.db_path)
    ...
    async def index_file(self, filepath: str) -> Optional[str]:
        """
        Indexes a single file by type. Routes all content through ChromaStore.

        Returns the document ID on success, None on failure or skip.
        """
        abs_path = os.path.abspath(filepath)
        ...
        if ext in (".txt", ".md") or basename.lower() in ("readme", "readme.md", "readme.txt"):
            try:
                ...
                if not content.strip():
                    return "empty"
                resp = self.store.add_document(content=content, container_tag=tag, filepath=abs_path)
        ...
        # 2. PDF — text + embedded images
            ...
            if text.strip():
                resp = self.store.add_document(content=text, container_tag=tag, filepath=abs_path)
                if resp:
                    sm_doc_id = resp.get("id")

            # 2b. Embedded image extraction → OCR → Chroma
        ...
        # 4. Images — OCR via pytesseract, skip if no text
            ...
            resp = self.store.add_image(filepath=abs_path, container_tag=tag)
            if resp:
                return resp.get("id")
