...
class LlamaCppProvider(LLMProvider):
    """
    Local llama.cpp server via its OpenAI-compatible /v1/chat/completions endpoint.
    Includes 'cache_prompt': True in the payload for prompt caching.
    """

    def __init__(self):
        self.base_url = os.environ.get("LLAMACPP_URL", "http://localhost:8080/v1")
        self.model    = os.environ.get("LLM_MODEL", "qwen2.5:3b")
        log.info(f"[LlamaCpp] model={self.model} url={self.base_url}")
    ...
    async def complete(
        ...
        log.debug(f"[LlamaCpp] raw → {content[:200]}")
        return {"content": content.strip(), "tool_calls": parsed_tools}
...
def create_provider() -> LLMProvider:
    ...
    LLM_BACKEND=llamacpp  → LlamaCppProvider
    """
    ...
    if backend == "groq":
        ...
        raise ValueError(
            f"Unknown LLM_BACKEND='{backend}'. Valid values: groq, ollama, llamacpp"
        )
