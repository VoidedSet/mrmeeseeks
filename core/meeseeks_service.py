...
class MeeseeksAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/search":
            ...
            self._send_json(chroma_store.get_profile(container_tag=tag))
    ...
    def _unified_search(self, query: str) -> List[Dict[str, Any]]:
        """
        Queries Chroma across all content containers.
        ...
        """
        grouped = chroma_store.search_all(query=query, container_tags=SEARCH_TAGS, limit=8)
    ...
    def _build_llm_context(self, query: str) -> Dict[str, Any]:
        ...
        doc_chunks = chroma_store.search_all(query=query, container_tags=SEARCH_TAGS, limit=5)
        ...
        memory_results = chroma_store.search_memories(query=query, container_tag="chat_memory", limit=4)
        memories = [m.get("content", "") for m in memory_results if m.get("content")]

        # User profile (cached — light call)
        profile_data = chroma_store.get_profile(container_tag="chat_memory", query=query)
    ...
    def _handle_store_memory(self, payload: Dict[str, Any]):
        """
        POST /memory — Store a conversation turn or direct fact into Chroma DB.
        ...
        if "conversationId" in payload:
            ...
            if not conv_id or not messages:
                ...
                return
            result = chroma_store.ingest_conversation(conversation_id=conv_id, messages=messages, container_tags=tags)
            ...
            result = chroma_store.add_memories(memories=memories, container_tag=tag, is_static=is_static)
            self._send_json(result or {"status": "error"})
    ...
    def _get_status(self) -> Dict[str, Any]:
        ...
        status: Dict[str, Any] = {
            ...
            "chroma_db_path": chroma_store.db_dir,
        }

        state_db = os.path.expanduser("~/.meeseeks/indexer_state.db")
