...
from subsystems.brain.brain import SentenceStreamer, brain
from subsystems.brain.state_machine import State
import subsystems.brain.llm_provider as llm_mod
...
async def main():
    from subsystems.brain.service_manager import service_manager
...
async def _main_impl():
    # Clear the profiler event log file
    profiler_emitter.clear()
    ...
    # ── Init LLM provider ────────────────────────────────────────────────────
    from subsystems.brain.llm_provider import init_provider
    try:
    ...
    # ── Register agents ──────────────────────────────────────────────────────
    from subsystems.tool_calling.agents.sysadmin_agent import register as reg_sysadmin
    reg_sysadmin()

    from subsystems.tool_calling.agents.memory_agent import register as reg_memory
    memory = reg_memory()

    from subsystems.tool_calling.agents.web_agent import register as reg_web
    reg_web()

    from subsystems.tool_calling.agents.hands_agent import register as reg_hands
    reg_hands()

    from subsystems.tool_calling.agents.eyes_agent import register as reg_eyes
    reg_eyes()

    if not args.cli:
        from subsystems.voice.voice_agent import register as reg_voice
        reg_voice()

    # ── Wire brain ───────────────────────────────────────────────────────────
    from subsystems.brain.brain import brain
    brain.inject_memory_agent(memory)

    # ── Start kernel listener (background task) ───────────────────────────────
    from subsystems.sentinel.kernel_listener import start as start_kernel
    ...
    if args.cli:
        ...
        # ── GUI Mode ─────────────────────────────────────────────────────────────
        from PyQt6.QtWidgets import QApplication
        from subsystems.brain.ui.overlay import UIOverlay
