...
from subsystems.brain.ui.design import Colors, Sizes
...
class BlueCursor(QWidget):
    ...
    Top-level frameless click-through window that:
      ...
      • renders as a metallic blue/cyan gradient sphere with breathing glow.
    """

    arrived = pyqtSignal()

    TRACKING_INTERVAL_MS = 8  # ~120fps polling
    TRACKING_SMOOTHING = 0.10  # 0 = frozen, 1 = snap. lower = smoother
    TRACKING_SNAP_THRESHOLD_PX = 0.25
    RESUME_TRACKING_DELAY_MS = 1500
    MOUSE_OFFSET = (4, 0)  # window offset from the OS cursor hotspot

    # Motion-stretch physics.
    STRETCH_MAX = 0.75
    STRETCH_DISTANCE_REFERENCE = 28.0
    STRETCH_FOLLOW = 0.30
    STRETCH_PERP_SQUISH = 0.8
    ANGLE_FOLLOW = 0.32
    ANGLE_MIN_GAP_PX = 1.0
